...
public class Kmeans {
    ...
    public static class kmeansMapperOne extends Mapper<LongWritable, Text, Text, Text> {
        ...
        public void setup(Context context) throws IOException, InterruptedException {
            String[] getCenterOne = ct.split(",");
            int count = 0;
            for (int i = 0; i < getCenterOne.length; i++) {
                centers[count][0] = Double.parseDouble(getCenterOne[i]);
                i++;
                centers[count][1] = Double.parseDouble(getCenterOne[i]);
                count++;
            }
        }
    ...
    public static class kmeansMapperTwo extends Mapper<LongWritable, Text, Text, Text> {
        ...
        static double[][] centers = new double[centerNum][2];
        /**
        ...
        public void setup(Context context) throws IOException, InterruptedException {
            ...
            URI u = URIs[0];
                ...
                String ppName = p.toString();
                String pFileName = p.getName().toString();
                String line = null;
                BufferedReader br = new BufferedReader(new FileReader(ppName));
                while ((line = br.readLine()) != null) {
                    //line = br.readLine();
                    //String[] rcd = line.toString().split("\t");
                    String[] rcd = line.split("\t");
                    String[] pair = rcd[0].split(",");
                    double x=Double.valueOf(pair[0]);
                    double y=Double.valueOf(pair[1]);
                    centers[count][0] = x;
                    centers[count][1] = y;
                    count++;
                }
...
//    }
    ...
    public static class KmeansReducerTwo extends Reducer<Text, Text, Text, Text> {
        @Override
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            double dis_x = 0.0;
            double dis_y = 0.0;
...
//        }
        ...
        job2.setReducerClass(KmeansReducerTwo.class);
    ...
    /**
     * The number of centers. Could be set by user.
     */
    private static int centerNum;

    /**
     * The input center string.
     */
    private static String ct;


    /**
     * Set the path for each main helper method.
     * Get the parameter for mainly use.
     * The input of HDFS should be :
     * hadoop jar xxx.jar inputPath outputPath times centers initialCenter(1.0,2.0,3.0,4.0).
     * Two double in a pair.
     */
    public static void main(String[] args) throws Exception {
        // Get main parameter.
        int times = Integer.parseInt(args[2]);
        centerNum = Integer.parseInt(args[3]);
